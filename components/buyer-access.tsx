...
import { ArrowLeft, FileText, Calendar, DollarSign, Copy, Search, Eye, CheckCircle } from "lucide-react"
...
export function BuyerAccess({ onBack, initialInvoiceId }: BuyerAccessProps) {
  ...
  const [highlightedInvoiceId, setHighlightedInvoiceId] = useState<string | null>(initialInvoiceId || null)
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [allInvoices, allProducts] = await Promise.all([getInvoices(), getProducts()])
          ...
          <div className="space-y-4">
            {filteredInvoices.map((invoice) => (
              <Card
                ...
                className={highlightedInvoiceId === invoice.id ? "ring-2 ring-primary" : ""}
              >
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                            ...
                            <div className="space-y-6">
                              <div className="grid grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <p className="font-mono font-medium">{selectedInvoice.id}</p>
                                </div>
                                <div>
                                      ...
                                      >
                                        <div>
                                          ...
                                          <p className="text-sm text-muted-foreground">
                                            ...
                                            {product ? formatRupiah(product.selling_price) : formatRupiah(0)}
                                          </p>
                                        </div>
                                        <p className="font-semibold">
                                          {product
                                            ? formatRupiah(product.selling_price * item.quantity)
                                            : formatRupiah(0)}
                                        </p>
                                      </div>
                                    )
                      ...
                      <Button
                        ...
                        className={copiedInvoiceId === invoice.id ? "bg-green-50 border-green-200" : ""}
                      >
                ...
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Buyer Name</label>
                      <p className="text-lg font-semibold">{invoice.buyer_name}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-muted-foreground">Date Created</label>
                      ...
                      {invoice.products.map((item, index) => {
                        ...
                        return (
                          <div key={index} className="flex justify-between items-center p-3 bg-muted rounded-lg">
                            ...
                            <p className="font-semibold">
                              {product ? formatRupiah(product.selling_price * item.quantity) : formatRupiah(0)}
                            </p>
                          </div>
                        )
                      })}
                  ...
                  <div className="border-t pt-4">
                    <div className="flex justify-between items-center">
                      ...
                      <p className="text-2xl font-bold text-primary">{formatRupiah(invoice.total_amount)}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
